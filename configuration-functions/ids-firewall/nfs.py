...
nfs_configuration_model = api.model('Server Configuration', {
    'ids': fields.String(required=True, description='Ids UUID', type='string'),
...
@nfs_ns.route('/ids', methods=['GET','PUT'])
class Nfs_Ids(Resource):
    @nfs_ns.response(200, 'Ids id retrieved.')
    @nfs_ns.response(500, 'Internal Error.')
    def get(self):
        # Get the id of ids
        try:
            json_data = json.dumps(NfsController().get_ids_id())
            resp = Response(json_data, status=200, mimetype="application/json")
            return resp
    ...
    @nfs_ns.param("Ids id", "Ids id to update", "body", type="string", required=True)
    @nfs_ns.response(202, 'Id correctly updated.')
    @nfs_ns.response(400, 'Bad request.')
    @nfs_ns.response(500, 'Internal Error.')
    def put(self):
        # Update the id of the ids
        try:
            id = request.data.decode()
            NfsController().set_ids_id(id)
            return Response(status=202)
